...
(function () {
    ...
    var u_class = '.gt_container-' + Array.from('globe' + wrapper_selector).reduce(function (h, c) {
    ...
    function gt_render_satelites(el) {
        gt_globe_open = true;
        el.querySelectorAll('.gsatelite img:not([src])').forEach(function (img) {
        ...
        var gs_pos = gsatelites.parentNode.style.position == 'fixed' && {
            top: gsatelites.offsetParent.offsetTop,
            left: gsatelites.offsetParent.offsetLeft
        } || {
            top: gsatelites.getBoundingClientRect().top,
            left: gsatelites.getBoundingClientRect().left
        };
        var c_pos = {
            top: gglobe.offsetTop + gglobe.clientHeight / 2,
            left: gglobe.offsetLeft + gglobe.clientWidth / 2
        };
        var count = languages.length,
            ...
            j = 0,
            angle = 0.75 * pi,
            orbit_count = Math.floor(2 * pi * r / dist),
            orbit_i = 0,
            delta = 2 * pi / orbit_count;
        el.querySelectorAll('.gsatelite').forEach(function (gs, i) {
            do {
                if (orbit_i >= orbit_count) {
                    r_num++;
                    r += r0;
                    orbit_i = 0;
                    orbit_count = Math.floor(2 * pi * r / dist);
                ...
                angle += delta;
                var x = c_pos.left + Math.cos(angle) * r - flag_size / 2,
                    y = c_pos.top + Math.sin(angle) * r - flag_size / 2;
                var vpHeight = window.innerHeight,
                    vpWidth = Math.floor(document.body.getBoundingClientRect().width),
                    tpViz = gs_pos.top + y >= 0 && gs_pos.top + y < vpHeight,
                    btViz = gs_pos.top + y + flag_size > 0 && gs_pos.top + y + flag_size <= vpHeight,
                    ltViz = gs_pos.left + x >= 0 && gs_pos.left + x < vpWidth,
                    rtViz = gs_pos.left + x + flag_size > 0 && gs_pos.left + x + flag_size <= vpWidth,
    ...
    function gt_hide_satelites() {
        gt_globe_open = false;
        var gsatelite = document.querySelectorAll('.gsatelite');
        gsatelite.forEach(function (e, i) {
            setTimeout(function () {
                e.style.display = 'none';
            }, (gsatelite.length - i - 1) * 10);
        });
    }
    ...
    document.querySelectorAll(u_class + ' .gglobe').forEach(function (e) {
        ...
        e.addEventListener('pointerenter', function (evt) {
            evt.target.parentNode.querySelectorAll('.gsatelite img:not([src])').forEach(function (img) {
                img.setAttribute('src', img.getAttribute('data-gt-lazy-src'))
            })
        });
    });
    if (url_structure == 'none') {
        ...
        function load_tlib() {
            if (!window.gt_translate_script) {
                ...
                gt_translate_script.src = 'js/element.js?cb=googleTranslateElementInit2';
