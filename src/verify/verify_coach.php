...
if(isset($email) && isset($v_code)){
    $query = "SELECT * FROM coach_registration where email='". $email ."' AND verification_code='". $v_code ."'";
    $result = mysqli_query($conn,$query);
    if($result){
        if(mysqli_num_rows($result)==1){
             $result_fetch = mysqli_fetch_assoc($result);
             if($result_fetch['is_verified'] == 0){
                $update = "UPDATE coach_registration SET is_verified='1' WHERE email='".$result_fetch['email']."'";
